...
namespace LCardAnalizator.graphicks
{
    static public class ZedGraphHelper
    {
            ...
            string title)
        {
            ...
            {
                myPane.CurveList.RemoveAll(curve => curve.Label.Text == name);
                LineItem myCurveSup = myPane.AddCurve(name, xPoints, yPoints, style.color, style.type);
                ...
                myCurveSup.Symbol.Fill.Color = style.color;
            ...
            LineItem myCurve = myPane.AddCurve("", x, y, System.Drawing.Color.Blue, SymbolType.None);
            ...
            double[] InEng)
        {
            ...
            myPane.CurveList.RemoveAll(curve => curve.Label.Text == "Минимум");
            ...
            // Обводка ромбиков будут рисоваться голубым цветом (Color.Blue),
            // Опорные точки - ромбики (SymbolType.Diamond)
            myPane.CurveList.RemoveAll(curve => curve.Label.Text == "Максимум");
            ...
            string name_minum)
        {
            ...
            if (in_tinf != null)
            {
                ...
                // Обводка ромбиков будут рисоваться голубым цветом (Color.Blue),
                // Опорные точки - ромбики (SymbolType.Diamond)
                myPane.CurveList.RemoveAll(curve => curve.Label.Text == name_minum);
            ...
            double[] y)
        {
            if (y != null)
            {
                ...
                // Обводка ромбиков будут рисоваться голубым цветом (Color.Blue),
                // Опорные точки - ромбики (SymbolType.Diamond)
                pane.CurveList.RemoveAll(curve => curve.Label.Text == "События");
            ...
            string name, StylePoint style)
        {
            if (y != null)
            {
                ...
                // Обводка ромбиков будут рисоваться голубым цветом (Color.Blue),
                // Опорные точки - ромбики (SymbolType.Diamond)
                pane.CurveList.RemoveAll(curve => curve.Label.Text == name);
                LineItem myCurve = pane.AddCurve(name, list, style.color, style.type);
                ...
                // !!!
                // Цвет заполнения отметок (ромбиков) - колубой
                myCurve.Symbol.Fill.Color = style.color;
